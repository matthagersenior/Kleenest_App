...
export default function BusinessIntelligenceActions({ businessId, items = [], locations = [], onComplete }) {
  ...
  async function runAction(item) {
    ...
    setBusy(item.location_id); setMessage(null);
    try {
  ...
  return <section className="detail-panel business-card"><div className="panel-heading"><div><span className="eyebrow">INTELLIGENCE ACTIONS</span><h2>Turn signals into action</h2></div><ShieldCheck size={22}/></div><div className="business-intelligence-list">{actionable.slice(0, 8).map((item) => {const type=item.recommendation.type;const label=type==='demand_opportunity'?'Create promotion':type==='operational_attention'?'Verify location':'Review route';const Icon=type==='demand_opportunity'?Megaphone:type==='operational_attention'?CheckCircle2:RouteIcon;return <div className="business-row" key={`${item.location_id}-${type}`}><div><strong>{item.recommendation.title} · {item.name}</strong><span>{item.recommendation.body}</span><span>{item.recommendation.reasons?.join(' · ') || 'Derived from current intelligence'}</span></div><button className="secondary" disabled={busy===item.location_id} onClick={()=>runAction(item)}><Icon size={16}/>{busy===item.location_id?'Working…':label}</button></div>})}</div>{message&&<p className="observation-copy" role="status">{message}</p>}</section>;
}
