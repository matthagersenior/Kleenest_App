...
import { ChevronDown,Database,Edit3,Plus,RefreshCw,Save,ShieldCheck,Trash2,X } from 'lucide-react';
import { ADMIN_RESOURCES,createAdminRow,deleteAdminRow,listAdminRows,updateAdminRow } from '../services/adminCrud';
import { useAuth } from '../context/AuthContext';
import { hasCapability } from '../domain/capabilities';
import '../styles/admin-crud.css';

const KEY_FIELDS={business_members:['business_id','user_id'],location_hours:['id'],location_amenities:['location_id','amenity_id'],favorites:['user_id','location_id'],follows:['follower_id','following_id'],user_badges:['user_id','badge_id'],level_definitions:['level'],location_confidence:['location_id'],business_members:['business_id','user_id'],notification_preferences:['user_id'],location_sources:['id'],location_verification_targets:['campaign_id','location_id'],subscription_plans:['id'],progression_actions:['id'],progression_games:['id'],progression_challenges:['id']};
const keyFor=(table,row)=> (KEY_FIELDS[table]||['id']).map(k=>row?.[k]).join('|');
...
export default function AdminCrudPage(){
 ...
 async function save(){setBusy(true);setError('');try{const payload=parsePayload(draft);if(editor){const keys=KEY_FIELDS[table]||['id'];if(keys.some(k=>editor[k]==null))throw new Error('This resource has no editable primary-key mapping yet.');let queryPayload={...payload};for(const k of keys)delete queryPayload[k];await updateAdminRow(table,keys.length===1?editor[keys[0]]:editor[keys[0]],queryPayload)}else await createAdminRow(table,payload);setEditor(null);await load()}catch(e){setError(e.message||String(e))}finally{setBusy(false)}}
 async function remove(row){if(!window.confirm(`Delete this ${label.toLowerCase()} record? This cannot be undone.`))return;setBusy(true);setError('');try{const keys=KEY_FIELDS[table]||['id'];if(keys.length!==1)throw new Error('This composite-key resource requires a dedicated relationship editor.');await deleteAdminRow(table,row[keys[0]]);await load()}catch(e){setError(e.message||String(e))}finally{setBusy(false)}}
 ...
 {editor&&<div className="crud-overlay" role="dialog" aria-modal="true"><div className="crud-modal"><div className="crud-modal-head"><div><span className="eyebrow">{editor.id?'EDIT':'CREATE'}</span><h2>{label}</h2></div><button className="icon-button" onClick={()=>setEditor(null)}><X/></button></div><textarea value={draft} onChange={e=>setDraft(e.target.value)} spellCheck="false"/><div className="crud-modal-actions"><button className="secondary" onClick={()=>setEditor(null}>Cancel</button><button className="primary" disabled={busy} onClick={save}><Save size={16}/> {busy?'Saving…':'Save changes'}</button></div></div></div>}
 </section>;
}
