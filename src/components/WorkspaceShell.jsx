...
import { getWorkspaceForMembership, getWorkspaceModel, MEMBERSHIP_UI, resolveWorkspace, WORKSPACE_NAVIGATION } from '../domain/workspaces.js';

function firstEntitlement(entitlements = []) {
    ...
    <div className={`app-shell workspace-shell workspace-${model.workspace.id}`} data-membership={membershipKnown ? model.membership : 'free'}>
      <header className="topbar workspace-topbar">
        ...
        <div className="header-actions workspace-actions">
          ...
          <button className="icon-button menu-button" type="button" onClick={() => setMenuOpen((open) => !open)} aria-label={menuOpen ? 'Close navigation' : 'Open navigation'} aria-expanded={menuOpen}>
      ...
      <footer className="workspace-footer">
        <span>Kleenest</span>
        <span>{getWorkspaceForMembership(model.membership) === model.workspace.id ? model.membershipLabel : model.workspace.label}</span>
      </footer>
    </div>
  );
}
