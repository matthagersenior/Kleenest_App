...
import { Database, RefreshCw, MapPinned, Layers, MapPinCheck } from 'lucide-react';
...
export default function AdminDataPage(){
 ...
 if(!hasCapability(capabilities,'admin'))return <section className="page"><div className="empty-state"><h2>Administrator access required</h2><p>This surface is protected by Supabase Auth + server-side capability checks.</p></div></section>;
 ...
 <button className="primary" disabled={!!busy} onClick={()=>run('addresses',()=>backfillLocationAddresses(25))}><MapPinCheck size={18}/> {busy==='addresses'?'Backfilling addresses…':'Backfill 25 addresses'}</button>
