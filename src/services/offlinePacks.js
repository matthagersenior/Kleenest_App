...
export async function createOfflinePack({kind='area',name='Kleenest offline area',routeId=null,routeDiscoverySessionId=null,businessId=null,bounds=null,expiresHours=PACK_TTL_HOURS}={}){
  ...
  const {data,error}=await supabase.rpc('create_offline_pack',{
    p_pack_type:pPackType,
    p_name:name,
    p_business_id:businessId,
    p_route_discovery_session_id:routeDiscoverySessionId,
    p_west:bounds?.west??null,
    p_south:bounds?.south??null,
    p_east:bounds?.east??null,
    p_north:bounds?.north??null,
    p_expires_hours:expiresHours,
  });
  if(error)throw error;
  const pack=Array.isArray(data)?data[0]:data;
  if(!pack?.id)return pack;
  const [{data:locations},{data:businesses}]=await Promise.all([
    supabase.from('offline_pack_locations').select('location_id,snapshot,source_version,cached_at').eq('pack_id',pack.id),
    supabase.from('offline_pack_businesses').select('business_id,snapshot,source_version,cached_at').eq('pack_id',pack.id),
  ]);
...
export async function syncOfflineEvents(){
  ...
  for(const event of events){
    const {error}=await supabase.from('offline_pack_events').upsert({id:event.id,pack_id:event.pack_id,event_type:event.event_type,payload:event.payload,client_event_id:event.client_event_id,created_at:event.created_at,synced_at:new Date().toISOString()},{onConflict:'client_event_id'});
    if(error){failed++;continue;}
    synced++;
    const db=await openDb();
    await new Promise((resolve,reject)=>{const tx=db.transaction('events','readwrite');tx.objectStore('events').put({...event,synced_at:new Date().toISOString()});tx.oncomplete=resolve;tx.onerror=()=>reject(tx.error);});
    db.close();
  }
