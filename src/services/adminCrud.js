...
export async function deleteAdminRow(table,record){const client=requireSupabase();assertAdminResource(table);await applyKeys(client.from(table).delete(),table,record);const {data,error}=await applyKeys(client.from(table).select('count').limit(1),table,record);if(error)throw error;return {ok:true};}
