...
export async function listMyRoutes({status=null,limit=25}={}){
  ...
  let query=supabase.from('route_plans').select(ROUTE_FIELDS).eq('user_id',user.id).order('created_at',{ascending:false}).limit(limit);
...
export async function getRoute(routeId){
  const user=await requireUser();
  const {data,error}=await supabase.from('route_plans').select(`${ROUTE_FIELDS},route_stops(id,location_id,stop_order,points_value,checked_in_at,created_at)`).eq('id',routeId).eq('user_id',user.id).single();
  if(error) throw error;
  return data;
}

export async function createRoute({name='My route',startLat=null,startLng=null,endLat=null,endLng=null,routeGeometry=null,distanceMiles=0,estimatedMinutes=0}={}){
  await requireUser();
  const {data,error}=await supabase.rpc('create_route_plan',{
    p_name:name,
  ...
  if(error) throw error;
  const routeId=Array.isArray(data)?data[0]?.id||data[0]:data;
  if(!routeId) throw new Error('Route creation did not return a route id.');
...
export async function prepareRouteOfflinePack(routeId,{corridorMeters=1000,name='Route offline network',expiresHours=24}={}){
  await requireUser();
  const session=await prepareRouteDiscovery(routeId,{corridorMeters});
  const sessionId=Array.isArray(session)?session[0]?.id:session?.id;
  if(!sessionId) throw new Error('Route discovery did not return a session.');
  return createOfflinePack({kind:'route',name,routeDiscoverySessionId:sessionId,expiresHours});
}
...
export async function completeRoute(routeId){
  await requireUser();
  const {data,error}=await supabase.rpc('complete_route',{p_route_id:routeId});
  if(error) throw error;
  const result=Array.isArray(data)?data[0]:data;
  return result?.id ? getRoute(result.id) : getRoute(routeId);
}
