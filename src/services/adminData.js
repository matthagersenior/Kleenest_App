...
async function invoke(name,body){
  ...
  if(error){let detail=error.context?.body||error.message;if(typeof detail!=='string')detail=JSON.stringify(detail);throw new Error(`${name}: ${detail}`)}
  if(data?.ok===false||data?.error)throw new Error(data.error||`${name} returned an error.`);
  return data;
}
export async function runDataIngest(action,payload={}){return invoke('public-data-ingest-v4',{action,...payload})}
...
export async function ingestRegion(region,{sources=['osm','data_gov']}={}){const city=String(region).toUpperCase();const results={};if(sources.includes('osm'))results.osm=await runNetworkIngest({source:'osm',markets:[city]});if(sources.includes('data_gov'))results.data_gov=await runNetworkIngest({source:'datagov',markets:[city]});return results}
