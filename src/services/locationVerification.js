...
export async function submitLocationVerification({ placeId, locationId=null, isOpen, isPublic, notes='' }){
  ...
  const payload={location_id:canonicalId,user_id:user.id,is_open:Boolean(isOpen),is_public:Boolean(isPublic),notes:String(notes||'').trim()||null,observed_at:new Date().toISOString(),source:'community'};
  const queued=await queueIfOffline(payload);
  if(queued)return {queued_offline:true,event:queued,...payload};
  const { data, error } = await client().from('location_verification_observations').upsert(payload, { onConflict:'location_id,user_id' }).select().single();
  if(error) throw new Error(error.message||'Verification could not be saved.');
  return data;
}

export async function listLocationVerificationSummary(placeIdOrLocationId){const id=await canonicalLocationId(placeIdOrLocationId).catch(()=>placeIdOrLocationId);const { data, error } = await client().from('location_verification_observations').select('id,user_id,is_open,is_public,observed_at,source,notes').eq('location_id',id).order('observed_at',{ ascending:false }).limit(50);if(error) throw new Error(error.message||'Verification data could not be loaded.');const rows=data||[];return { observations:rows, total:rows.length, openCount:rows.filter(r=>r.is_open).length, publicCount:rows.filter(r=>r.is_public).length, lastObservedAt:rows[0]?.observed_at||null };}
