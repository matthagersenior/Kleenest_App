...
// Canonical check-in entrypoint. The database RPC atomically creates the
// check-in, reward transaction, and profile reward totals.
...
export async function listMyCheckIns() {
  ...
  const { data, error } = await supabase
    .from('check_ins')
    .select('id,location_id,qr_code_id,checked_in_at,verification_method,points_awarded,metadata')
    .eq('user_id', user.id)
