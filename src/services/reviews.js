...
export async function createReview({locationId,stars,comment,cleanlinessPct=null,checkInId=null}={}){
  ...
  const queued=await queueIfOffline({locationId,stars:normalizedStars,comment:String(comment||'').trim(),cleanlinessPct:cleanliness,checkInId});
  if(queued)return {queued_offline:true,event:queued};
  const {data,error}=await supabase.rpc('create_review',{p_location_id:locationId,p_check_in_id:checkInId,p_stars:normalizedStars,p_cleanliness_pct:cleanliness,p_comment:String(comment||'').trim()});
  if(error) throw error;
