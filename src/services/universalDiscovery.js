...
export async function discoverUniversalLocations({ latitude, longitude, radiusKm = 50, userId = undefined } = {}) {
  ...
  const { data, error } = await supabase.rpc('prepare_universal_location_discovery', {
    p_lat: Number(latitude),
    p_lng: Number(longitude),
    p_radius_m: Math.round(Math.min(Math.max(radiusKm, 1), 50) * 1000),
    p_user_id: userId ?? undefined,
  });
  if (error) throw error;
  const rows = Array.isArray(data?.locations) ? data.locations : [];
  return rows.filter(row => Number.isFinite(Number(row.latitude)) && Number.isFinite(Number(row.longitude))).map(row => normalizePlace({
    ...row,
    location_id: row.id,
    ...
    is_verified: row.bathroom_verification_status === 'verified' || row.verification_status === 'verified',
  }));
}
