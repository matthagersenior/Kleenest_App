...
export function normalizeMembership(raw = {}) {
  ...
  if (tier.includes('admin')) return 'admin'; if (tier.includes('fleet')) return 'fleet';
  if (tier.includes('enterprise')) return tier.includes('business') ? 'business_enterprise' : 'enterprise';
  if (tier.includes('growth')) return 'business_growth'; if (tier.includes('standard') && tier.includes('business')) return 'business_standard';
  if (tier.includes('family')) return 'family'; if (tier.includes('premium') || tier.includes('pro')) return 'premium'; return 'free';
}
export const MEMBERSHIP_UI = Object.freeze({
  free: Object.freeze({ label: 'Free', ads: true, workspace: 'consumer' }), premium: Object.freeze({ label: 'Premium', ads: false, workspace: 'consumer' }),
  family: Object.freeze({ label: 'Family', ads: false, workspace: 'consumer' }), business_standard: Object.freeze({ label: 'Business Standard', ads: false, workspace: 'business' }),
  business_growth: Object.freeze({ label: 'Business Growth', ads: false, workspace: 'business' }), business_enterprise: Object.freeze({ label: 'Business Enterprise', ads: false, workspace: 'enterprise' }),
  enterprise: Object.freeze({ label: 'Enterprise', ads: false, workspace: 'enterprise' }), fleet: Object.freeze({ label: 'Fleet', ads: false, workspace: 'fleet' }),
  admin: Object.freeze({ label: 'Admin', ads: false, workspace: 'admin' }),
});
...
export function resolveWorkspace(pathname = '/', capabilities = []) {
  const path = String(pathname || '/');
  const candidate = path.startsWith('/admin') ? 'admin' : path.startsWith('/enterprise') ? 'enterprise' : path.startsWith('/fleet') ? 'fleet' : path.startsWith('/business') ? 'business' : 'consumer';
...
export function resolveMembership(rawMembership, capabilities = []) {
  ...
  if (hasCapability(capabilities, CAPABILITIES.ADMIN)) return 'admin'; if (hasCapability(capabilities, CAPABILITIES.FLEET)) return 'fleet';
  if (hasCapability(capabilities, CAPABILITIES.ENTERPRISE)) return 'enterprise'; if (hasCapability(capabilities, CAPABILITIES.BUSINESS)) return 'business_standard'; return parsed;
}
export function getWorkspaceModel({ membership = 'free', workspace, businessId = null, capabilities = [] } = {}) {
  const resolvedMembership = resolveMembership(membership, capabilities); const requestedWorkspace = workspace || getWorkspaceForMembership(resolvedMembership);
  const resolvedWorkspace = canUseWorkspace(capabilities, requestedWorkspace) ? requestedWorkspace : 'consumer';
  return Object.freeze({ membership: resolvedMembership, membershipLabel: MEMBERSHIP_UI[resolvedMembership]?.label || 'Free', adsEnabled: MEMBERSHIP_UI[resolvedMembership]?.ads ?? true, workspace: getWorkspace(resolvedWorkspace), businessId, availableWorkspaces: getAvailableWorkspaces(capabilities) });
}
