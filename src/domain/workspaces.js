...
export const WORKSPACE_NAVIGATION = Object.freeze({
  consumer: Object.freeze([
    ...
    { id: 'activity', label: 'Activity', path: '/profile' },
    { id: 'play', label: 'Play', path: '/games' },
...
const CAPABILITY_FOR_WORKSPACE = Object.freeze({
...
export function getAvailableWorkspaces(capabilities = []) {
  return WORKSPACE_ORDER.filter((workspace) => canUseWorkspace(capabilities, workspace));
}

export function normalizeMembership(raw = {}) {
  const source = raw?.data || raw?.entitlement || raw?.entitlements || raw;
  const tier = String(source?.membership_tier || source?.product_tier || source?.tier || source?.service_tier || source?.plan || 'free')
    .toLowerCase()
    .replace(/[-\s]/g, '_');
  if (tier.includes('admin')) return 'admin';
  if (tier.includes('fleet')) return 'fleet';
  if (tier.includes('enterprise')) return tier.includes('business') ? 'business_enterprise' : 'enterprise';
  if (tier.includes('growth')) return 'business_growth';
  if (tier.includes('standard') && tier.includes('business')) return 'business_standard';
  if (tier.includes('family')) return 'family';
  if (tier.includes('premium') || tier.includes('pro')) return 'premium';
  return 'free';
}

export const MEMBERSHIP_UI = Object.freeze({
  free: Object.freeze({ label: 'Free', ads: true, workspace: 'consumer' }),
  premium: Object.freeze({ label: 'Premium', ads: false, workspace: 'consumer' }),
  family: Object.freeze({ label: 'Family', ads: false, workspace: 'consumer' }),
  business_standard: Object.freeze({ label: 'Business Standard', ads: false, workspace: 'business' }),
  business_growth: Object.freeze({ label: 'Business Growth', ads: false, workspace: 'business' }),
  business_enterprise: Object.freeze({ label: 'Business Enterprise', ads: false, workspace: 'enterprise' }),
  enterprise: Object.freeze({ label: 'Enterprise', ads: false, workspace: 'enterprise' }),
  fleet: Object.freeze({ label: 'Fleet', ads: false, workspace: 'fleet' }),
  admin: Object.freeze({ label: 'Admin', ads: false, workspace: 'admin' }),
});

export function getWorkspaceForMembership(membership) {
  return MEMBERSHIP_UI[membership]?.workspace || 'consumer';
}
...
export function getWorkspaceModel({ membership = 'free', workspace, businessId = null, capabilities = [] } = {}) {
  ...
  return Object.freeze({
    ...
    membershipLabel: MEMBERSHIP_UI[resolvedMembership]?.label || 'Free',
    adsEnabled: MEMBERSHIP_UI[resolvedMembership]?.ads ?? true,
    workspace: getWorkspace(resolvedWorkspace),
    businessId,
