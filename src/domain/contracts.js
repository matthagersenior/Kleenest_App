...
export function normalizePlace(place){
  ...
  return {
    id:String(place.id),
    location_id:place.location_id?String(place.location_id):null,
    ...
    cleanliness:place.cleanliness??null,
    cleanliness_pct:place.cleanliness_pct==null?null:Number(place.cleanliness_pct),
    accessible:Boolean(place.accessible),
    changing_table:Boolean(place.changing_table),
    smart_bathroom:Boolean(place.smart_bathroom),
    bathroom_verification_status:place.bathroom_verification_status??null,
    bathroom_verification_count:Number(place.bathroom_verification_count??0),
    bathroom_positive_count:Number(place.bathroom_positive_count??0),
    bathroom_negative_count:Number(place.bathroom_negative_count??0),
    source:place.source??null,
    source_dataset:place.source_dataset??null,
    freshness:place.updated_at??place.created_at??null,
  };
}

export function cleanlinessLabel(place){
  if(place?.cleanliness_pct!=null){
    if(place.cleanliness_pct>=85)return 'Excellent';
    if(place.cleanliness_pct>=70)return 'Good';
    if(place.cleanliness_pct>=50)return 'Fair';
