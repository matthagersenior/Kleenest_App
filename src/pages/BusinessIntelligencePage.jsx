...
export default function BusinessIntelligencePage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      if (!business) {
        if (requestId === requestRef.current) setState({ business: null, locations: [], intelligence: [] });
        return;
      }
      const [locations, intelligence] = await Promise.all([
        listLocations(business.id),
        getBusinessLocationIntelligence(business.id)
      ]);
