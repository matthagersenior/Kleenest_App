...
| Evidence | Location evidence + contribution services | visits, observations, amenities, restroom quality, verification, photos, reviews | Place, Map intelligence, Reputation, Business evidence | record/submit/verification/photo/review RPCs | Evidence panels / contribution actions |
...
| Offline | Offline packs / queues | cached locations, queued observations/arrivals/reviews | Map, Evidence, Route, Place | offline pack/discovery contracts | Offline controls |
...
| Business | Business services | managed locations, campaigns, promotions, events, analytics, ROI, review response | Business dashboard, intelligence actions | business analytics/action/review RPCs | Business controls |
...
3. Evidence is shared intelligence, not a separate competing category system. Verification, quality, amenities, restroom observations, photos, visits, and reviews enrich the same location record.
...
11. Review creation is a consumer capability contract (`create_review`); business review analytics and replies are business capability contracts. Service wrappers must not create alternate RPC dispatch paths.
...
`platformCapabilities.js` is the cross-domain RPC contract registry. It covers consumer, evidence, routing, live network, gamification, QR, enterprise, Fleet, Business, and Admin, including the review creation/analytics/reply contracts. New service modules should consume these canonical contracts where practical instead of creating duplicate RPC wrappers.

### Data interoperability

The highest-risk duplication points are location identity, route identity, evidence state, review state, and notification state. These must remain keyed to Supabase canonical IDs and contracts. Do not normalize them into independent client-side domain stores that can drift.
